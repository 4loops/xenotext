...
def single_compatable(single):
    ...
    for x in range(26):
        for y in range(x+1,26):
            if single[0] != x and single[1] != y and single[0] != y and single[1] != x:
...
for x,y in single_ciphers_c:
    ...
    beam.append((key,(np.packbits(single_ciphers_c[(x,y)]),np.dot(s_vec,single_ciphers_c[(x,y)]))))
...
while not np.all(np.any(cipher_unkey(beam[0][0]),axis=0)):
    ...
    count += 1
    next_beam = {}
    for old_key, (comp,score) in beam:
